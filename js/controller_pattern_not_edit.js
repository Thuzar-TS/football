...
.controller("logoutController", function($location) {
      ...
      window.localStorage.removeItem("uniqUserId");
...
.controller("memberController", function($scope, $http, $location) {
  if (window.localStorage.getItem("userId")) {
    ...
    if (window.localStorage.getItem("userRoleId")) {
      $("#wrapper").css("display","block");
           if (window.localStorage.getItem("userRoleId")!=3) {
            checkuser();
                if (window.localStorage.getItem("userRoleId")==2) {
                    $("#accmenu").css("display","block");
                  }
                  else if (window.localStorage.getItem("userRoleId")==1) {
    ...
    function checkuser(){
      $http.post("query.php",{"type":"checkmember","btn":"user","mid":window.localStorage.getItem("uniqUserId"),"roleid":window.localStorage.getItem("userRoleId")})
      .then(function (response) {
           if (response.data.alluser == "no") {
            window.localStorage.removeItem("userRoleId");
            window.localStorage.removeItem("uniqUserId");
            $("#wrapper").css("display","none");  
            $("#adminmenu").css("display","none");  
     ...
     if (window.localStorage.getItem("userRoleId")==2) {
       ...
       /* $scope.onoffarray=[{onoff_id:"1",onoff_name:"Login OFF",onoff_type:"loginonoff"},
          ...
          /*$scope.calculatecom = function(a){
            $http.post("query.php",{"type":"comformula","btn":"checkcomcalmix","mix":a.mixname})
            .then(function (response) {
              if (response.data.calculated == 1) {
                    alert("It's already caculated.");
                  }
                  else{
                    var com = confirm("Are you sure to calculate Mix Commission?");
                    if (com == true) {
                      $http.post("query.php",{"type":"comformula","btn":"mix", "mix":a.mixname})
                          .then(function (response) {
                            $scope.comledger = response.data.comledger;
                            if ($scope.comledger != "No Record") {
                              $scope.calcom($scope.comledger,a.mixname);
                            }
                            else{
                              alert("No Commission Record.");
                            }
                          })
                    }
                  } 
          ...
          $scope.calcom = function(dd,aa){             
              var resultval = 0;
              for (var i =dd.length - 1; i >= 0; i--) {
                if (parseInt(dd[i].net_amount) > 0) {
                  resultval = parseInt(dd[i].net_amount)*dd[i].mixwinper;
                }
                else if (parseInt(dd[i].net_amount) == 0){
                  resultval = parseInt(dd[i].bet_amount)*dd[i].mixloseper;
                }
                  $scope.comarr.push({result:resultval, lgrid:dd[i].ledger_id, agcomid:dd[i].agent_commission_id, agmbrid:dd[i].agentmemberid});                                        
              }
              $http.post("query.php",{"type":"addcommission","btn":"update", "comtype":"mix", "mix":aa, "arraydata":$scope.comarr, "userid":window.localStorage.getItem("uniqUserId")})
              .then(function (response) {
                mainfun();
                alert(response.data);
              })
          ...
          $scope.changeCom = function(a){
            var ch = confirm("Are you sure to change commission formula to normal state ?\nMix Name => "+a.mixname);
            if (ch == true) {
              $http.post("query.php",{"type":"chformula","btn":"com", "sub":"mix", "dashid":a.mixname, "userid":window.localStorage.getItem("uniqUserId")})
                  .then(function (response) {
                    alert(response.data);
                    mainfun();
                  })
...
.controller("userController", function($scope,$http,$location) {
  ...
  else if(window.localStorage.getItem("userRoleId")==1 || window.localStorage.getItem("userRoleId")==2){
    checkuser();
    if (window.localStorage.getItem("userRoleId")==2) {
      $("#accmenu").css("display","block");
    }
...
.controller("mhomeController", function($scope, $filter, $http, $routeParams, $location, $interval) {
  ...
  $scope.dd = new Date();
  $scope.currenty = parseInt($scope.dd.getFullYear());
   ...
   var month=$scope.d.getMonth()+1;
    ...
    if (window.localStorage.getItem("userRoleId")!=3) {
      checkuser();
       if (window.localStorage.getItem("userRoleId")==2) {
        ...
        $scope.rid = window.localStorage.getItem("userRoleId");

                if ($scope.rid == 3) {
                      $scope.m = window.localStorage.getItem("uniqUserId");
                      $scope.mid = "and a.member_id="+window.localStorage.getItem("uniqUserId");
                      if ($routeParams.datevalue) {   
                        ...
                        if ($routeParams.mbrid) {
                                ...
                                $scope.mbrid = " and a.member_id="+window.localStorage.getItem("uniqUserId");
                            }
           ...
           // if ($scope.mix == "no") {
            ...
            //  }
              ...
              $scope.changeyear = function(a){
                  var concon = confirm("Are you sure to change year in search data?");
                  if (concon == true) {
                    mhomecall(a.year);
                  }
              }
              ...
              $scope.changemixdetail = function(){
                ...
                $scope.mix = "mix";
                $scope.datevaluefilter = $scope.todayval;
                mhomecall($scope.chyearcombo.year);
              }
    ...
    function mhomecall(aa){
        $http.post("query.php",{"chyear":aa, "type":"mpage","mid":$scope.mid,"btn":"ledger","sub":"","datevalue":$scope.datevalue,"mbrid":$scope.mbrid, "mix":$scope.mix})
        .then(function (response) {
                $scope.alldata = response.data;
                $scope.total = response.data.m;
                $scope.yeararr = response.data.yeararr;
    ...
    function checkmbr(){
      $http.post("query.php",{"type":"checkmember","btn":"member","mid":window.localStorage.getItem("uniqUserId")})
      .then(function (response) {
  ...
  $scope.filterDate = $filter('date')(Date.now(), 'MM-dd-yyyy');
      ...
      if (window.localStorage.getItem("userRoleId")!=3) {  
          $location.path("/");
            ...
            $http.post("query.php",{"type":"detailledger","btn":"","mid":window.localStorage.getItem("uniqUserId")})
            .then(function (response) {
 ...
 if (window.localStorage.getItem("userRoleId")) {
  ...
  if (window.localStorage.getItem("userRoleId")!=3) {
    ...
    function filterallfun(){
             ...
             if($scope.allfilter.statusfilter!=undefined)
              ...
              $scope.depoself = function(a){
                    ...
                    if (a == "s") {
                      ...
                      $http.post("query.php", {"type":"checkpercent","btn":"depocheck","mbrid":window.localStorage.getItem("uniqUserId")})
                      .then(function(response){
                        if (response.data.onoffbet == "can") {
                           if ($scope.amt == null || $scope.amt == "" || $scope.cdnum == null || $scope.cdnum == "" || $scope.card == null || $scope.card == "" || $scope.usrname == null || $scope.usrname == "" || $scope.bname == null || $scope.bname == "" || $scope.bankselect == null || $scope.bankselect == undefined || $scope.phone == null || $scope.phone == "" || $scope.city == null || $scope.city == "" || $scope.branch == null || $scope.branch == "") {
                              ...
                              if (parseInt($scope.amt) < parseInt(response.data.min)) {
                                alert("At least "+response.data.min+" ks deposit.");
                                $scope.depodisable = false;
                              }
                              else if(parseInt($scope.amt) > parseInt(response.data.max)){
                                alert("Beyond Limit! Up to "+response.data.max+" ks can deposit.");
                                $scope.depodisable = false;
                              }
                              else{
                                $scope.reqdeposite();
                        ...
                        else if (response.data.onoffbet == "cannot"){
                          alert("Sorry, you cannot Deposit now. Please contact with site admin.");
                          checkonoff();
                        }    
                      })                                                     
                    }                   
                    ...
                    //alert($scope.mid+"/"+$scope.amt+"/"+$scope.bankselect.bank_history_id+"/"+$scope.cdnum+"/"+$scope.card+"/"+$scope.usrname+"/"+$scope.phone+"/"+$scope.city+"/"+$scope.branch+"/"+$scope.m);
                        ...
                        .then(function (response) {
                             ...
                             // $scope.allfilter.filterBank = $scope.bank[0];
                              ...
                              $scope.bankselect = undefined;
                              $scope.depodisable = false;
                              if (response.data.transfercan == "can") {
                                alert("Request Sent");
                              }
                              else if (response.data.transfercan == "cannot") {
                                alert("Sorry, you can't Deposite now. Please contact with site admin.");
                              }
                              checkonoff()
                          });
                    }
      ...
      function checkonoff(){
        $http.post("query.php",{"mid":window.localStorage.getItem("uniqUserId"),"type":"onoff","onofftype":"depositonoff","btn":"check"})
        .then(function (response) {
    ...
    $scope.depoself = function(a){
      if (a == "s") {
       //   $scope.usrname = $scope.total[0].username;
        ...
        $scope.depodisable = true;
        $http.post("query.php", {"type":"checkpercent","btn":"withcheck","mbrid":window.localStorage.getItem("uniqUserId")})
        .then(function(response){
          if (response.data.onoffbet == "can") {
             if ($scope.mbankselect != undefined) {
              ...
              if ($scope.amt == null || $scope.amt == "" || $scope.cdnum == null || $scope.cdnum == " " || $scope.cdnum == "" || $scope.usrname == null || $scope.usrname == "" || $scope.realbname == null || $scope.realbname == undefined || $scope.phone == null || $scope.phone == "" || $scope.city == null || $scope.city == "" || $scope.branch == null || $scope.branch == "") {
                alert("Please Fill All Data");
                 ...
                 if (parseInt($scope.amt) < parseInt(response.data.min)) {
                  alert("At least "+response.data.min+" ks withdraw.");
                  $scope.depodisable = false;
                }
                else if(parseInt($scope.amt) > parseInt(response.data.max)){
                  alert("Beyond Limit! Up to "+response.data.max+" ks can withdraw.");
                  $scope.depodisable = false;
                }
                else{
                  $scope.reqwithdraw();
            ...
            alert("Sorry, you cannot Withdraw now. Please contact with site admin.");
            checkonoff();
          }    
        })        
            ...
            if (parseInt($scope.amt)<parseInt($scope.total[0].amount)==false) {
                ...
                .then(function (response) {
                      ...
                      if (response.data.transfercan == "can") {
                        ...
                        alert("Sorry, you can't Withdraw now. Please contact with site admin.");
                      }  
                      checkonoff();
                      });
                    }                                         
      ...
      function checkonoff(){
        $http.post("query.php",{"mid":window.localStorage.getItem("uniqUserId"),"type":"onoff","onofftype":"withdrawonoff","btn":"check"})
        .then(function (response) {
          if (response.data.accepttype == "can") {
            $scope.deporeq = true;
          }
...
.controller("transferController", function($scope, $filter, $http, $location, Excel, $timeout) {
 ...
 $scope.$watchCollection('allfilter', function(newValue, oldValue) {
    ...
    $scope.allchange = function(a){
        //alert(a.amount+"/"+a.member_id+"/"+a.state_id+"/"+a.allid+"/"+a.alltype);
          ...
          .then(function (response) {
              ...
              $scope.mbank = response.data.mbks;
              $scope.minlimit = response.data.minlimit; 
              $scope.maxlimit = response.data.maxlimit;       
              filterallfun();
          }); 
      }
      ...
      $scope.closefun = function(){
        ...
        $scope.depodisable = false;
        $scope.trancode = null;
        $scope.transfertext = null;        
      }

      $scope.sendcode = function(a){
        $("#logincode").css("display","none");
        $("#black-overlay").css("display","none");

        if (a == "mbr") {          
          $http.post("query.php", {"userid":window.localStorage.getItem("uniqUserId"), "pass":$scope.trancode, "btn":"usercode", "type":"checktransfer", "sub":"mbr"})
          .then(function(response){
            if (response.data == 1) {
              $http.post("query.php", {"type":"checkpercent","btn":"transfercheck","mbrid":window.localStorage.getItem("uniqUserId")})
              .then(function(response){
                if (response.data.onoffbet == "can") {
                     if ($scope.amt == null || $scope.amt == "" || $scope.transferid == null || $scope.transferid == "") {
                        ...
                        $scope.trancode = null;
                         ...
                         if (parseInt($scope.amt) < parseInt(response.data.min)) {
                            alert("At least "+response.data.min+" ks can transfer.");
                            $scope.depodisable = false;
                            $scope.trancode = null;
                          }
                          else if(parseInt($scope.amt) > parseInt(response.data.max)){
                            alert("Beyond Limit! Up to "+response.data.max+" ks can transfer.");
                            $scope.depodisable = false;
                            $scope.trancode = null;
                          }
                          else{
                  ...
                  alert("Sorry, you cannot Transfer now. Please contact with site admin.");
                  $scope.closefun();
                }    
              })          
            }
            else{
              alert("Your password's Incorrect.");
              $scope.trancode = null;
              $scope.depodisable = false;
            }
          })
          ...
          $http.post("query.php", {"userid":window.localStorage.getItem("uniqUserId"), "pass":$scope.trancode, "btn":"usercode", "type":"checktransfer", "sub":"admin"})
          .then(function(response){
            if (response.data == 1) {
        ...
        $http.post("query.php",{"testfinid":$scope.transferid,"type":"testing","btn":"finuniqueid"})
          ...
          if (parseInt($scope.amt)<parseInt($scope.total[0].amount)==false) {
                ...
                $scope.depodisable = false;
                  ...
                  if (parseInt($scope.amt)<=$scope.maxlimit ==false){
                      alert("Maximun Transfer Amount is "+$scope.maxlimit);
                      $scope.depodisable = false;
                  }
                  else if (parseInt($scope.amt)>=$scope.minlimit ==false) {
                      alert("Minimun Transfer Amount is "+$scope.minlimit);
                      $scope.depodisable = false;
                  }
                  else{
                      ...
                      $http.post("query.php",{"type":"mpage", "amt":$scope.amt, "transferid":$scope.transferid, "oramt":$scope.total[0].amount, "mid":$scope.mid, "mbrid":$scope.m, "transfertext":$scope.transfertext, "btn":"allwdt","inner":"reqtransfer","sub":"transfer"})
                        .then(function (response) {
                            ...
                            if (response.data.transfercan == "can") {
                              ...
                              alert("Sorry, you can't Transfer now. Please contact with site admin.");
                            }  
                            filterallfun();
                            checkonoff();                      
                        });
  ...
  function checkonoff(){
    $http.post("query.php",{"mid":window.localStorage.getItem("uniqUserId"),"type":"onoff","onofftype":"transferonoff","btn":"check"})
    .then(function (response) {
...
.controller("mledgerController", function($scope, $filter, $http, $location, $routeParams, $timeout, Excel) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
       ...
       checkuser();
        ...
        $("#header").css("display","block");
            ...
            if ($scope.rid == 3) {
              ...
              if ($routeParams.mid) {
                $scope.mid = "and a.member_id="+$routeParams.mid;
                $scope.userview = true;
              }
              else{
                $scope.mid = "and 1=1";
          ...
          $scope.exportToExcel=function(tableId){ // ex: '#my-table'
            var exportHref=Excel.tableToExcel(tableId,'User All Ledger');
            $timeout(function(){location.href=exportHref;},100); // trigger download
        }
...
.controller("allledgerController", function($scope, $http, $filter, $log, $location) {
  ...
  if (window.localStorage.getItem("roleId")) {
   ...
   if (window.localStorage.getItem("userRoleId")) {
  $("#wrapper").css("display","block");
  if (window.localStorage.getItem("userRoleId")==1 || window.localStorage.getItem("userRoleId")==2) {
    checkuser();
    if (window.localStorage.getItem("userRoleId")==2) {
      $("#accmenu").css("display","block");
    }
...
.controller("accdashController", function($scope, $rootScope, $http, $location, $log, $route,$interval, Excel,$timeout){
  ...
  $scope.comarr = [];
   if (window.localStorage.getItem("userRoleId")) {    
      ...
      /*  
        if (window.localStorage.getItem("userRoleId")==1) {
              $scope.adminshow=true; 
        }
        else if(window.localStorage.getItem("userRoleId")==2){
 ...
 /*  $interval(function(){                  
    getdata();
              ...
              $scope.calculatecom = function(a){
                $http.post("query.php",{"type":"comformula","btn":"checkcomcal","dashid":a.dashboard_id})
                .then(function (response) {
                  if (response.data.calculated == 1) {
                    alert("It's already caculated.");
                  }
                    ...
                    if (com == true) {
                      $http.post("query.php",{"type":"comformula","btn":"bg", "dashid":a.dashboard_id, "abody":a.abody, "agoal":a.agoal})
                          .then(function (response) {
                            $scope.forname = response.data.forname;
                            $scope.comledger = response.data.comledger;
                            $scope.agoalfor = response.data.agoalfor;
                            $scope.abodyfor = response.data.abodyfor;
                            //$scope.agcom = response.data.agcom;

                            if ($scope.comledger != "No Record") {
                              $scope.calcom($scope.abodyfor,$scope.agoalfor,$scope.forname,$scope.comledger,a.dashboard_id);
                            }
                            else{
                              alert("No Commission Record.");
                            }
              ...
              $scope.calcom = function(aa,bb,cc,dd,ee){                 
                  var resultval = 0;
                  for (var i =dd.length - 1; i >= 0; i--) {
                    if (dd[i].bet == "Body") {
                      var str = String(aa);
                      ...
                      if (dd[i].accbgid != 0) {
                        str = str.replace(/Beton/g,dd[i].bet_on);
                        for (var j =cc.length - 1; j >= 0; j--) {
                          var ff = cc[j];
                          if (ff == "Body%") {
                              str = str.replace(/Body%/g,dd[i].body_value/100);
                          }
                          else if (ff == "Goal%") {
                              str = str.replace(/Goal%/g,dd[i].goalplus_value/100);
                          }
                          else if (ff == "Stake") {
                            str = str.replace(/Stake/g,dd[i].bet_amount);
                          }
                          else if (ff == "AgentLose") {
                            str = str.replace(/AgentLose/g,dd[i].loseper);
                          }
                          else if (ff == "AgentWin") {
                            str = str.replace(/AgentWin/g,dd[i].winper);
                          }
                        }
                  ...
                  $http.post("query.php",{"type":"addcommission","btn":"update", "comtype":"bg", "dashid":ee, "arraydata":$scope.comarr, "userid":window.localStorage.getItem("uniqUserId")})
                  .then(function (response) {
                    getdata();
                    alert(response.data);
                  })
                ...
                $scope.changeGoalformula = function(a,b){
                  ...
                  if (ch == true) {
                    $http.post("query.php",{"type":"chformula","btn":"goal", "dashid":a, "userid":window.localStorage.getItem("uniqUserId")})
                        .then(function (response) {
                          alert(response.data);
                          getdata();
                        })
                ...
                $scope.changeCom = function(a,b){
                  var ch = confirm("Are you sure to change commission formula to normal state ?\nMatch => "+b.Home+" & "+b.Away);
                  if (ch == true) {
                    $http.post("query.php",{"type":"chformula","btn":"com", "sub":"bg", "dashid":a, "userid":window.localStorage.getItem("uniqUserId")})
                        .then(function (response) {
                          alert(response.data);
                          getdata();
                        })
                ...
                $scope.changeBodyformula = function(a,b){
                  ...
                  if (ch == true) {                    
                    $http.post("query.php",{"type":"chformula","btn":"body", "dashid":a, "userid":window.localStorage.getItem("uniqUserId")})
                        .then(function (response) {
                          alert(response.data);
                          getdata();
                        })
    ...
    function getdata(){
        $http.post("query.php",{"type":"accdash","btn":"","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD, "datesign":$scope.datesign, "userid":window.localStorage.getItem("uniqUserId")})
         //$http.post("query.php",{"type":"accdash","btn":""})
          .then(function (response) {
              ...
              $scope.abody.splice(0,0, { accbody_id: "0", description: "Choose", formula: ""});     
              $scope.agoals.splice(0,0, { accgoal_id: "0", description: "Choose", formula: ""});  
    ...
    $scope.pinall = function(){
        ...
        if (c == true) {
          ...
          /* for (var i =ar.length - 1; i >= 0; i--) {            
           ...
           $http.post("query.php",{"type":"checkmixmatch","btn":"pinall", "arraydata":ar, "userid":window.localStorage.getItem("uniqUserId")})            
            .then(function(response){ 
            })
        }
    }
        ...
        if (c == true) { 
               if (a == 'io') {
                 ...
                 for (var i =ar.length - 1; i >= 0; i--) {            
                  ...
                  $http.post("query.php",{"type":"accdash","fromdate":"","todate":"","otherdate":"", "datesign":"","btn":"ioallchange", "arraydata":ar, "userid":window.localStorage.getItem("uniqUserId")})            
                  .then(function(response){ 
                ...
                $http.post("query.php",{"type":"accdash","fromdate":"","todate":"","otherdate":"", "datesign":"","btn":"mixchange", "arraydata":ar, "userid":window.localStorage.getItem("uniqUserId")})            
                  .then(function(response){      
                    $scope.iochangealldata = response.data;
                    $scope.dashes = response.data.dashboard;
                    $scope.bodies = response.data.bodies;
                ...
                $http.post("query.php",{"type":"accdash","fromdate":"","todate":"","otherdate":"", "datesign":"","btn":"comchange", "arraydata":ar, "userid":window.localStorage.getItem("uniqUserId")})            
                  .then(function(response){      
                    $scope.iochangealldata = response.data;
                    $scope.dashes = response.data.dashboard;
                    $scope.bodies = response.data.bodies;
    ...
    $scope.changeMixFor = function(dashid,x){
      var conmixfor = confirm("Are You Sure To Change Mix Formula?");
          if (conmixfor == true) {
              $http.post("query.php",{"type":"changemixfor","btn":"mix","dashid":dashid})
              .then(function (response) {
                alert(response.data);
              })
              getdata();
    ...
    $scope.calculateMixFor = function(dashid,x){
      $scope.mixbetarr = [];
      var conmixfor = confirm("Are You Sure To Calculate Mix Formula?");
          if (conmixfor == true) {
              $http.post("query.php",{"type":"changemixfor","btn":"calculate","dashid":dashid, "bid":x.accbody_id, "gid":x.accgoal_id})
              .then(function (response) {
                $scope.forname = response.data.forname;
    ...
    $scope.savemix = function(){
      $http.post("query.php", {"type":"addmledger", "arraydata":"No Record", "mixarrdata":$scope.mixbetarr})
          .then(function(response){
           alert("Success!");
           getdata();
          })
        ...
        if ($scope.accgoalval.accgoal_id != 0 || $scope.accbodyval.accbody_id != 0) {
          ...
          window.localStorage.setItem("timetableid",  $scope.tid);
            ...
            .then(function(response){
              ...
              formulafuncall($scope.arr,$scope.bformula,$scope.gformula,$scope.forname,$scope.mixbody,$scope.mixgoal,$scope.mixarr);
  ...
  $scope.savedash = function(){
   if ($scope.accgoalval.accgoal_id != 0 || $scope.accbodyval.accbody_id != 0) {
    ...
    $http.post("query.php",{"type":"accdash","fromdate":"","todate":"","otherdate":"", "datesign":"", "bless":$scope.bless, "gless":$scope.gless, "blimitamt":$scope.blimitamt, 
      ...
      "score":$scope.scoreval,"btn":"save" , "userid":window.localStorage.getItem("uniqUserId")})
    ...
    function formulafuncall(arr,bformula,gformula,forname,mixbody,mixgoal,mixarr){
      if (arr != "No Record") {
                    for (var i =arr.length - 1; i >= 0; i--) {   
                      if (arr[i].bet == "Body") {
                          arr[i].accbgid = $scope.accbodyval.accbody_id;
                          if (bformula != "No Record") {
                            cboselectchangefor(arr[i],bformula,forname);
                          }
                          
                      }
                      else if(arr[i].bet == "Goal+"){
                          arr[i].accbgid = $scope.accgoalval.accgoal_id;
                          if (gformula != "No Record") {
                            cboselectchangefor(arr[i],gformula,forname);
                          }                  
                      }                    
                }
              }
              if (mixarr != "No Record") {
                mixbgformula(mixarr,mixbody,mixgoal,forname);
              }
    }
...
.controller("mixdetailController", function($scope, $http, $filter, $location,$interval, $log, $routeParams, Excel, $timeout){
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
...
.controller("DashCtrl", function($scope, $http, $location,$interval, $log){  
  ...
  if (window.localStorage.getItem("roleId")) {
    ...
    //$("#testtb").css("height","68vh");
    //$("#testtb").css("height","100%");
    //$("tr").css("height", "10%");
            ...
            if (window.localStorage.getItem("userRoleId")) {
              $scope.roleidshow = true;
                   if (window.localStorage.getItem("userRoleId")!=3) {
                    checkuser();
                    ...
                    else if(window.localStorage.getItem("userRoleId")==3){
                      $scope.changeshow = true;
                      checkmbr();
                      checkonoff();
                        $("#header").css("display","block");
                      ...
                      $http.post("query.php",{"type":"onoff","btn":"createuseronoff","mid":0})
                      .then(function (response) {
                        $scope.creatembr = response.data.creatembr;
                        if (response.data.creatembr == false) {
                          $scope.roleidshow = true;              
                        
                          $("#login").css("display","block");
                        }
                       else{
                          $scope.roleidshow = false;                        
                          
                          $("#login").css("display","block");
                          $("#userdash").css("display","none");
                          $("#login").addClass("col-sm-offset-4");
    ...
    function checkonoff(){
      $http.post("query.php",{"mid":window.localStorage.getItem("uniqUserId"),"type":"onoff","onofftype":"bettingonoff","btn":"check"})
      .then(function (response) {
    ...
    function checkrunning(){
        $http.post("query.php",{"type":"checkrunning","btn":"","mid":window.localStorage.getItem("uniqUserId")})
        .then(function (response) {
 ...
 Array.prototype.unique = function() {
  ...
  $scope.hhh = function(a,b,c){
    $scope.checkarr.push({dashid:a,hper:b.hper,aper:b.aper,uper:b.uper,dper:b.dper,body_value:b.body_value,goalplus_value:b.goalplus_value});
      $scope.checkarr = $scope.checkarr.unique();
      ...
      goalval:b.goalplus_value,bodyname:b.bodyname,goalname:b.goalname,mix:b.mix_on_off, beton:c});
  ...
  $scope.bettingmix = function(){
      ...
      $http.post("query.php", {"type":"checkpercent","btn":"mixcheck","arraydata":$scope.mixlength, "mbrid":window.localStorage.getItem("uniqUserId")})
      .then(function(response){
   
      $scope.chdata = response.data.chdata;
      if (response.data.breakbet) {
        if (response.data.onoffbet == "can") {
          for (var i = $scope.mixlength.length - 1; i >= 0; i--) {
            $scope.mixlength[i].beton = $scope.mixdiv[$scope.mixlength[i].dashid];
          }

          $scope.allbet = parseInt(response.data.membertype[0].totalamt)-parseInt(response.data.totalbet);
          if (parseInt($scope.allbet) > 0) {
            if ($scope.betamount == null || $scope.betamount == "" || $scope.betamount == 0) {
             $scope.errormsg = "Please Add Amount"; 
             $scope.testing = false;
            }
            else{
                if (parseInt($scope.betamount) <= parseInt($scope.allbet)) {
                  ...
                  alert("Sorry, you can bet only "+$scope.allbet+" ks today.");
                  $scope.testing = false;
                }
            }           
            }
          else{
            alert("Sorry, you can't bet today.\nYour limit is full.\nPlease try at the next day.");
            checkonoff();
            $scope.bDone();
          }        
        }
        else if (response.data.onoffbet == "cannot") {
          alert("Sorry, you cannot bet now. Please contact with site admin.");
          checkonoff();
          $scope.bDone();
        }
      }
      else{
        alert("Sorry, you are choosing close match. Please try again.");
          checkonoff();
  ...
  $scope.checkmixodd = function(){
    for (var i = $scope.checkarr.length - 1; i >= 0; i--) {
        if ($scope.chdata[i].aper == $scope.checkarr[i].aper) {
          if ($scope.chdata[i].body_value == $scope.checkarr[i].body_value) {
            if ($scope.chdata[i].dper == $scope.checkarr[i].dper) {
              if ($scope.chdata[i].goalplus_value == $scope.checkarr[i].goalplus_value) {
                if ($scope.chdata[i].hper == $scope.checkarr[i].hper) {
                  if ($scope.chdata[i].uper == $scope.checkarr[i].uper) {
                    $scope.arrequal = "equal";
                  }
                  else{
                    $scope.arrequal = "notequal";
                    break;
                  }
                }
                else{
  ...
  $scope.mixreal = function(){
    var conbet = confirm("Are you sure this betting?");
    if (conbet == true) {
      $http.post("query.php",{"type":"bettingmix","amount":$scope.betamount, "mid":window.localStorage.getItem("uniqUserId"), "mix":$scope.mixnumber, "arraydata":$scope.mixlength})
      .then(function (response) {
  ...
  function getdata(){
    if (window.localStorage.getItem("uniqUserId") == 1) {
        $scope.admbr = true;
     }
     else{
        $scope.admbr = false;
    ...
    $scope.closefun = function(){
      ...
      $scope.errorTxt = "";
      $scope.username = null;
      $scope.logincode = null;  
      $scope.password = null;
      getdata();
    }

     $scope.sendcode = function(){
        $http.post("query.php", {"username":$scope.username, "pass":$scope.password, "type":"login", "logincode":$scope.logincode})
        .then(function(response){
          $scope.alldata = response.data;
          ...
          if (response.data.roleid!=null) {
            ...
            if ($scope.roleId!=3) {
              ...
              window.localStorage.setItem("userRoleId",  $scope.roleId);
              window.localStorage.setItem("uniqUserId",  $scope.userId);  
              $scope.mid = "and 1=1";                
            }
            $("#black-overlay").css("display","none");                      
            $("#logincode").css("display","none");
    ...
    $scope.login = function(){
          ...
          if ($scope.username==null && $scope.password==null) {
              ...
              $http.post("query.php", {"username":$scope.username, "pass":$scope.password, "type":"login", "logincode":null})
              .then(function(response){
                   $scope.alldata = response.data;
                   if (response.data == 1) {
                    $("#black-overlay").css("display","block");                      
                        ...
                        $("#header").css("display","block");
                          if ($scope.roleId!=3) {
                            ...
                            $("#navbar-data1 .mmenu").css("display","none");
                              ...
                              if (response.data.mixbetonoff == "can") {
                                $scope.onlyhome1 = true;
                              } 
                              else{
                                $scope.onlyhome1 = false;   
    ...
    $scope.betmember = function(b){
      $scope.testing = true;
      $http.post("query.php", {"dashid":b, "type":"checkpercent","btn":"check","bg":$scope.betbody,"mbrid":window.localStorage.getItem("uniqUserId")})
        .then(function(response){
     
          if (response.data.chdata != "No Record") {
            if (response.data.onoffbet == "can") {
            $scope.allbet = parseInt(response.data.membertype[0].totalamt)-parseInt(response.data.totalbet);
            if (parseInt($scope.allbet) > 0) {
              if (parseInt($scope.amount) <= parseInt($scope.allbet)) {
                $scope.betreal(response.data.chdata,b,response.data.membertype);
              }
              else{
                alert("Sorry, you can bet only "+$scope.allbet+" ks today.");
                $scope.testing = false;
          ...
          alert("Sorry, this match is finished.");
          checkonoff();
          $scope.bDone();
        }
        })   
      ...
      $scope.betreal = function(chdata,b,c){
      ...
      $scope.testing = true;
        $scope.m = window.localStorage.getItem("uniqUserId");
             if($scope.betbody == true){
              ...
              if (c[0].typeid !=0) {
                $scope.bless = c[0].minbet;
                $scope.gless = c[0].minbet;
              }
            ...
            //if (confirmupdate == true) {
                if (parseInt($scope.amount)<parseInt($scope.bless)) {
                  ...
                  var conbet = false;
                    ...
                    if (c[0].typeid != 0) {
                      if (parseInt($scope.amount)>parseInt(c[0].maxbet)) {
                        $scope.errormsg = "Please Bet at most "+c[0].maxbet+" ks";
                        $scope.blimitamt = c[0].maxbet;
                        var conbet = false;
                      }
                    }
                    ...
                    .then(function (response) {
                        if (response.data.can == 1) {
                          $scope.joinagentid = response.data.agentid;
                           var c = confirm("Confirm agent name : "+response.data.agentname+"\n(Agent ID : "+$scope.agentfinanceid+")");
                            if (c == true) {
                              testloginid();
                            }                          
...
.controller("mixbetlistController", function($rootScope, $scope, $http, $interval, $filter, $location, Excel, $timeout, $routeParams) {
  ...
  $scope.dd = new Date();
    $scope.currenty = parseInt($scope.dd.getFullYear());
    //$scope.chyear.year = $scope.currenty;
    ...
    if (c == true) {
        var ar = $scope.mixledger.filter(
           ...
           $http.post("query.php",{"type":"mixbetlist","btn":"chbetstate", "arrdata":ar,"fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD, "datesign":$scope.datesign})
            ...
            $scope.mixarray = response.data.mixdata; 
            $scope.mixarray.splice(0,0, {mix_id:0,mixname:"-- Calculate Com --"});       
            $scope.comcal = $scope.mixarray[0];  
  ...
  $scope.calculatemix = function(a,b,c){
    ...
    for(var i = 0; i<b; i++){   
      ...
      if (checkstatus == 1 && mbr != 0) {
        $scope.resarr.push({mbrid:mbr,mixid:c[i],resval:mpp,turnval:turnover,resamtval:resamt,status:stid,ck:checkstatus});
      }      
      mpp = 1;   
    }
  ...
  function getmixbetlistdata(aa){  
      $scope.wltotal = 0;
      $scope.turntotal = 0;
      $scope.staketotal = 0;
      $http.post("query.php",{"chyear":aa, "type":"mixbetlist","btn":"","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD, "datesign":$scope.datesign})
      .then(function (response) {
        $scope.alldata = response.data;
        $scope.yeararr = response.data.yeararr;
        $scope.mixledger = response.data.mixledger;
        ...
        //$scope.chyearcombo.year = $scope.currenty;     
...
.controller("userlistController", function($rootScope, $scope, $http, $interval, $filter, $location, Excel, $timeout, $routeParams) {
      ...
      $("#wrapper").css("display","block");
        ...
        function filterallfun(){
          ...
          function getalldaydata(aa){
            $scope.alldayfilter = true;
            $scope.adshow = 3;
            $scope.wltotal = 0;
            $scope.turntotal = 0;
            $scope.staketotal = 0;
            $http.post("query.php",{"chyear":aa, "type":"allusrlist","btn":"alllist","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD, "datesign":$scope.datesign})
            .then(function (response) {
                $scope.alldata = response.data;
                $scope.mledger = response.data.ledger;
                $scope.yeararr = response.data.yeararr;
        ...
        function getusrlistdata(aa){
          $scope.wltotal = 0;
          $scope.turntotal = 0;
          $scope.staketotal = 0;
          $http.post("query.php",{"chyear":aa, "type":"usrlist","betstateid":0, "tid":window.localStorage.getItem("timetableid"),
            "btn":"","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD, "datesign":$scope.datesign})
          .then(function (response) {
              $scope.alldata = response.data;
              $scope.yeararr = response.data.yeararr;
...
.controller("mprofileController", function($filter, $scope, $http, $log, md5, $location, $routeParams, Excel, $timeout) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
  ...
  if (window.localStorage.getItem("userRoleId")!=3) {
    ...
    checkmbr();
      ...
      $("#login").css("display","none");
            ...
            else if (window.localStorage.getItem("uniqUserId")) {
                $scope.mid = window.localStorage.getItem("uniqUserId");
            }
            $scope.cardbtn = true;
            ...
            $scope.bankhis = 0;
              $http.post("query.php",{"type":"mpage","mid":$scope.mid,"btn":"mprofile","sub":""})
                  .then(function (response) {
                      ...
                      /*$scope.usertype.splice(0,0, {typeid:'0',typename:"-- Default --"});*/
                      $scope.membertype = $filter('filter')($scope.usertype,{typeid:String(response.data.mp[0].typeid)}, true)[0];
                   //   $scope.bankselect = $scope.bks[0];
                       ...
                       if ($scope.agenttest == 1) {
                          ...
                          $scope.onoffagentcom = response.data.commission;
                          $scope.onoffagent = response.data.agentonoff;
                       }
              });
          ...
          $scope.agenton = function(){
            ...
            if (agenton == true) {
              $scope.agentdisable = true;              
             $http.post("query.php",{"type":"mpage","mid":$scope.mid,"btn":"mprofile","sub":"agentsetup","userid":window.localStorage.getItem("uniqUserId")})
                  .then(function (response) {
          ...
          $scope.changemembertype = function(a){
            $http.post("query.php",{"type":"mpage","mid":$scope.mid,"btn":"mprofile","sub":"chmembertype","userid":window.localStorage.getItem("uniqUserId"),"typeid":a.typeid})
            .then(function (response) {
              $scope.mp = response.data.mp;
              $scope.agenttest = response.data.agent;
              $scope.financeid = $scope.mp[0].financeid;
          ...
          $scope.editagent = function(a){
            ...
            $("#black-overlay").css("display","block"); 
            $scope.winper = parseFloat($scope.agentdata[0].winper);
            $scope.loseper = parseFloat($scope.agentdata[0].loseper);
            $scope.mixwinper = parseFloat($scope.agentdata[0].mixwinper);
            $scope.mixloseper = parseFloat($scope.agentdata[0].mixloseper);
          }

          $scope.editagentval = function(a){
            if ($scope.winper == null || $scope.loseper == null || $scope.mixwinper == null || $scope.mixloseper == null || $scope.onoffagent == null || $scope.onoffagentcom == null) {
              alert("Please Fill Completely.");
            }
            else{
              $http.post("query.php",{"type":"mpage", "mid":$scope.mid, "btn":"mprofile", "sub":"editagent", "winper":$scope.winper, "loseper":$scope.loseper, "mixloseper":$scope.mixloseper, "mixwinper":$scope.mixwinper,
               "onoffag":$scope.onoffagent, "onoffagcom":$scope.onoffagentcom})
              .then(function (response) {
                $scope.mp = response.data.mp;
                $scope.agenttest = response.data.agent;
                $scope.financeid = $scope.mp[0].financeid;
          ...
          $scope.profilebetlist = function(a){            
              ...
              .then(function (response) {
                $scope.allbetlist = response.data.bgbetlist;
                $scope.mixledger = response.data.mixbetlist;
                if ($scope.allbetlist == "No Record") {
                  $scope.allbetlist = [];
                }
                if ($scope.mixledger == "No Record") {
                  $scope.mixledger = [];
                }
            ...
            $scope.hashpass = md5.createHash($scope.oldpass);
              ...
              if ($scope.hashpass == $scope.mp[0].password) {
                    if (($scope.pass == null || $scope.pass == "" || $scope.pass == undefined) && ($scope.conpass == null || $scope.conpass == "" || $scope.conpass == undefined)) {
                      ...
                      $http.post("query.php",{"mid":$scope.mid,"pass":$scope.pass,"type":"mpage", "btn":"mprofile","sub":"edit"})
                            .then(function (response) {
                                    $scope.mp = response.data.mp;
                                    $scope.bks = response.data.bank;
                                 //   $scope.bankselect = $scope.bks[0];
...
.controller("alluserController", function($log,$scope,$http,$location, Excel,$timeout) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
       ...
       if (window.localStorage.getItem("userRoleId")!=3) {
        ...
        $scope.creatememberoff = false;
          if (window.localStorage.getItem("userRoleId")==2) {
                  ...
                  $scope.onoffarray=[{onoff_id:"1",onoff_name:"Login OFF",onoff_type:"loginonoff"},
                  {onoff_id:"2",onoff_name:"BG Betting OFF",onoff_type:"bettingonoff"},
                  {onoff_id:"3",onoff_name:"Mix ON",onoff_type:"mixbettingonoff"},
                  {onoff_id:"4",onoff_name:"Deposit OFF",onoff_type:"depositonoff"},
                  {onoff_id:"5",onoff_name:"Withdraw OFF",onoff_type:"withdrawonoff"},
                  {onoff_id:"6",onoff_name:"Transfer OFF",onoff_type:"transferonoff"},
                  {onoff_id:"7",onoff_name:"Commission ON",onoff_type:"commissiononoff"}];
                  ...
                  $scope.exportToExcel=function(tableId){ // ex: '#my-table'
                    ...
                    $timeout(function(){location.href=exportHref;},100); // trigger download
                      ...
                      $scope.onofffilter = function(a){
                        ...
                        $http.post("query.php",{"type":"alluser", "btn":"onofffilter", "onofftype":aa})
                        .then(function (response) {
                          $scope.alldata = response.data;
                      ...
                      $scope.creatembroff = function(){    
                        if ($scope.creatememberoff == false) {
                          var con = confirm("Are you sure to off create new user?");
                        }
                        else{
                          var con = confirm("Are you sure to on create new user?");
                        }                 
                        
                        if (con == true) {
                          $http.post("query.php",{"type":"onoff", "btn":"newuseroff", "onoff":$scope.creatememberoff, "mid":""})
                          .then(function (response) {
                            $scope.creatememberoff = response.data.creatembr;
    ...
    if (window.localStorage.getItem("userRoleId")==1) {
      ...
      $scope.tlimitedit = true;
           ...
           $scope.editlimit = function(){
              if ($scope.minlimit != null && $scope.minlimit != "" && $scope.maxlimit != null && $scope.maxlimit != "") {
                $http.post("query.php",{ "type":"tranlimit", "btn":"edit", "min":$scope.minlimit, "max":$scope.maxlimit, "limittype":"transfer"})
                    .then(function (response) {
                      $scope.alldata = response.data;
                        $scope.names = response.data.tlimit;
                        alert("Edit Successful");
...
.controller("agentuserlistController", function($scope, $http, Excel, $routeParams, $timeout, $location){
  ...
  if (window.localStorage.getItem("roleId")) {
   ...
   if (window.localStorage.getItem("userRoleId")==1 || window.localStorage.getItem("userRoleId")==3) {
      if (window.localStorage.getItem("userRoleId")==1) {
        checkuser();
        $("#wrapper").css("display","block");
        $("#adminmenu").css("display","block");
        ...
        if ($routeParams.mid) {
          ...
          $http.post("query.php",{ "type":"agentuserlist", "btn":"", "mid":$routeParams.mid})
            .then(function (response) {
              ...
              $scope.agentonoff = response.data.agentonoff;   
          })
        }   
        else{
          $location.path("/");
        ...
        if(window.localStorage.getItem("agentId")){
            ...
            $http.post("query.php",{ "type":"agentuserlist", "btn":"", "mid":window.localStorage.getItem("uniqUserId")})
              .then(function (response) {
                $scope.alldata = response.data;
                $scope.auser = response.data.agentuserlist;
                $scope.agentonoff = response.data.agentonoff;   
                if ($scope.agentonoff == "off") {
...
.controller("agentuserbetController", function($scope, $filter, $http, Excel, $routeParams, $timeout, $location){
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
   ...
   if (window.localStorage.getItem("userRoleId")==1 || window.localStorage.getItem("userRoleId")==3) {
    ...
    $scope.maindata = function(aa){
      $http.post("query.php",{ "type":"agentuserbet", "btn":aa, "datefilter":$scope.filterDate, "mbrid":$routeParams.mbrid, "agid":$scope.agid})
        .then(function (response) {
          $scope.alldata = response.data;
          if (response.data.betlist == "No Record") {
            ...
            $scope.betlist1 = response.data.betlist;
          }
      })
    }

   $scope.maindata1 = function(aa){
    $http.post("query.php",{ "type":"agentuserbet", "btn":aa, "datefilter":$scope.filterDate, "mbrid":$scope.mbrid, "agid":window.localStorage.getItem("uniqUserId")})
        .then(function (response) {
          $scope.alldata = response.data;
          if (response.data.betlist == "No Record") {
            $scope.betlist = "";
...
.controller("usertypesetupController", function($scope, $http, $location) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
   ...
   if (window.localStorage.getItem("userRoleId")!=3) {
    checkuser();
     if (window.localStorage.getItem("userRoleId")==2) {
      ...
      $http.post("query.php",{"type":"usertype", "btn":""})
      .then(function (response) {
          $scope.names = response.data;
      });
      $scope.showtype=true;
      $scope.save = function(){
        if ($scope.tname == null || $scope.totalamt == undefined || $scope.minbet == undefined || $scope.maxbet == undefined) {
            alert("Please Fill Completely");
        } else{
            $http.post("query.php",{"typename":$scope.tname, "totalamt":$scope.totalamt, "minbet":$scope.minbet, "maxbet":$scope.maxbet, "type":"usertype", "btn":"save", "userid":window.localStorage.getItem("uniqUserId")})
            .then(function (response) {
                $scope.names = response.data;
                alert("Successful Added");
            });
            $scope.tname = null;
            $scope.totalamt = undefined;
            $scope.minbet = undefined;
            $scope.maxbet = undefined;
        }                    
      }

     $scope.deltype = function(a){
              var deleted = confirm("Are you sure to delete?");
              if ( deleted == true) {
                      $http.post("query.php",{"typeid":a, "type":"usertype", "btn":"delete"})
                      .then(function (response) {
                          $scope.names = response.data;
                          alert("Successful Deleted");
                      });
     ...
     $scope.edittype = function(a,b){
        $scope.tname = a.typename;
        $scope.totalamt = parseInt(a.totalamt);
        $scope.minbet = parseInt(a.minbet);
        $scope.maxbet = parseInt(a.maxbet);
        $scope.tid = b;
       ...
       $scope.eteam = function(b){
        if ($scope.tname == null || $scope.totalamt == undefined || $scope.minbet == undefined || $scope.maxbet == undefined) {
           ...
           $http.post("query.php",{"typename":$scope.tname, "totalamt":$scope.totalamt, "minbet":$scope.minbet, "maxbet":$scope.maxbet, "typeid":b, "type":"usertype", "btn":"edit"})
          .then(function (response) {
              $scope.names = response.data;
              alert("Successful Edited");
          });
...
.controller("yearchangeController", function($scope, $http, $location) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
  ...
  if (window.localStorage.getItem("userRoleId")) {  
   if (window.localStorage.getItem("userRoleId")==1) {
    ...
    $http.post("query.php",{"type":"yearchange","btn":"onload"})
    .then(function (response) {
    ...
    $scope.chyear = function(aa,bb){
      var con = confirm("Are you sure to change year "+aa+" => "+bb+" ?");
      if (con == true) {
        $http.post("query.php",{"type":"yearchange","btn":"change"})
        .then(function (response) {
          $scope.change = response.data.change;
          $scope.currentyear = response.data.currentyear;
          $scope.changeyear = parseInt($scope.currentyear)+1;
...
.controller("cpuController", function($scope, $http, $location, $interval) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
  ...
  if (window.localStorage.getItem("userRoleId")) {  
   if (window.localStorage.getItem("userRoleId")==1) {
    ...
    $interval(function(){                  
     ...
     function getdata(){
      $http.post("cpuinfo.php",{"type":"","btn":""})
      .then(function (response) {

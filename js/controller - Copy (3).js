          ...
          $scope.calcom = function(dd,aa){             
              ...
              for (var i =dd.length - 1; i >= 0; i--) {
                if (parseInt(dd[i].net_amount) > 0) {
                  resultval = parseInt(dd[i].net_amount)*dd[i].winper;
                }
                else if (parseInt(dd[i].net_amount) == 0){
                  resultval = parseInt(dd[i].bet_amount)*dd[i].loseper;
                }
...
/*.controller("commissionController", function($scope, $filter, $http, $location, $routeParams) {
  if (window.localStorage.getItem("userId")) {
    window.localStorage.removeItem("userId");
  }
  if (window.localStorage.getItem("roleId")) {
  ...
  if (window.localStorage.getItem("userRoleId")) {
      ...
      if (window.localStorage.getItem("userRoleId")!=3) {
       ...
       // maindata(window.localStorage.getItem("uniqUserId"));
     }
  }

  function maindata(a){
    $http.post("query.php",{"type":"commission","btn":"","mid":a})
    .then(function (response) {
...
if (window.localStorage.getItem("userRoleId")) {
  ...
  function datacall(){
      $http.post("query.php",{"type":"mpage","mid":$scope.mid,"btn":"wdt","sub":"","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD,"datesign":$scope.datesign})
        ...
        function checkmbr(){
              $http.post("query.php",{"type":"checkmember","btn":"","mid":window.localStorage.getItem("uniqUserId")})
                .then(function (response) {
...
.controller("mprofileController", function($filter, $scope, $http, $log, md5, $location, $routeParams) {
  ...
  if (window.localStorage.getItem("userRoleId")!=3) {
    ...
    $("#header").css("display","block");
          ...
          $scope.showallledger = function(a){
            $scope.mid = "and a.member_id="+a;
            $http.post("query.php",{"type":"mpage","mid":$scope.mid,"btn":"wdt","sub":"","fromdate":$scope.fromD,"todate":$scope.toD,"otherdate":$scope.otherD,"datesign":$scope.datesign})
            .then(function (response) {
...
.controller("agentuserbetController", function($scope, $http, Excel, $routeParams, $timeout, $location){
  ...
  if (window.localStorage.getItem("roleId")) {
   ...
   function maindata1(aa){
            $http.post("query.php",{ "type":"agentuserbet", "btn":aa, "mbrid":$scope.mbrid, "agid":window.localStorage.getItem("uniqUserId")})
                  .then(function (response) {
                    $scope.alldata = response.data;

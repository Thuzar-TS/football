...
$scope.calculatemix = function(a,b,c){
    ...
    for(var i = 0; i<b; i++){      
      for(var j = 0; j<a.length; j++){
            if (a[j].mixledger_id == c[i] && a[j].betstateid == 1) {
                  if (a[j].bgstatusid != 0 && a[j].bgstatusid != 2 && a[j].bgstatusid != 6 && a[j].status_id == 6) {
                    checkstatus = 1;
                  }
                  else{
                    checkstatus = 0;
      ...
      if (checkstatus == 1) {
        ...
        $scope.resarr.push({mbrid:mbr,mixid:c[i],resval:mpp,turnval:turnover,resamtval:resamt,status:stid, ch:checkstatus});
       /* $scope.resarr1.push({mbrid:mbr,mixid:c[i],resval:mpp,turnval:turnover,resamtval:resamt,status:stid,ch:checkstatus});
      console.log($scope.resarr1);*/
      }      
      mpp = 1;   
    }
